...
> Source: `data/dataset.csv` (4,569 rows, 29 columns, seed=42)
...
|-----------|--------------|-------------|-------------|-------------|--------|
| Workspace | `workspace` | `dim_workspace` | Client workspace / channel identifier | 5 | WS-DIGITAL-NEWS, WS-ENTERTAINMENT, WS-TECH-ANALYSIS, WS-LIFESTYLE, WS-SPORTS-LIVE |
...
| AI Output Type | `ai_output_type` | `dim_ai_output_type` | AI-generated content package (what MediaFlow created) | 5 | key_moments, chapters, full_package, summary, my_key_moments |
...
| Billable | `billable_flag` | True / False | MediaFlow billing status, independent of publish status |
...
|---------|----|----|--------|
| Channel × Input Type | workspace | input_type | video count / hours |
| Channel × Language | workspace | language | video count |
| User × Output Type | uploaded_by | ai_output_type | published count |
| Team × Platform | team_name | published_platform | impressions |
| Workspace × Published Status | workspace | published_flag | PCR % |
...
| 2 languages only | English/Hindi — less diverse than original MediaFlow data | LPI |
